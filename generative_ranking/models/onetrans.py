...
class MixedCausalAttention(nn.Module):
    def __init__(
        self, ns_len, d_model, num_heads=4, if_mask=True, mask_type="paper_causal"
    ):
        super().__init__()
        ...
        self.kqv_list = nn.ModuleList(
            [
                nn.ModuleList([nn.Linear(d_model, d_model) for _ in range(3)])
                for _ in range(ns_len + 1)
            ]
        )
    ...
    def cal_mix_param_kqv(self, x):
        return (
            self._project_one(x[0], 0),
            self._project_one(x[1], 1),
            self._project_one(x[2], 2),
        )
...
class OneTransBlock(nn.Module):
    def __init__(
        ...
        self.cma = MixedCausalAttention(
            ns_len=ns_len, d_model=d_model, num_heads=num_heads, mask_type=mask_type
        )
        self.ffn_list = nn.ModuleList(
            [FFNLayer(d_model, ffn_hidden, d_model) for _ in range(ns_len + 1)]
        )
...
class OneTrans(nn.Module):
    def __init__(
        ...
        if self.non_seq_input_dim <= 0:
            raise ValueError(
                "OneTrans requires non-sequential features for NS tokenization"
            )
        if self.seq_input_dim <= 0:
            raise ValueError(
                "OneTrans requires sequence feature embeddings for S tokenization"
            )

        self.non_seq_tokenizer = nn.Linear(
            self.non_seq_input_dim, self.ns_len * self.d_model
        )
    ...
    def _build_non_seq_inputs(self, x):
        ...
        for fea in self.item_sequence_features:
            ...
            if fea.pooling == "concat":
                raise ValueError(
                    "OneTrans does not support concat pooling for non-sequential item features"
                )
    ...
    def _build_seq_tokens(self, x):
        ...
        for fea in self.sequence_features:
            ...
            if fea.pooling != "concat":
                raise ValueError(
                    "OneTrans sequence features must use pooling='concat' to preserve token order"
                )
    ...
    def forward(self, x):
        ...
        ns_tokens = self.non_seq_tokenizer(non_seq_inputs).view(
            batch_size, self.ns_len, self.d_model
        )
