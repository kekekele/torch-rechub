...
def normalize_rankmixer_group_schema(group_schema, default_seq_pool_modes=("mean",)):
    ...
    for index, raw_group in enumerate(group_schema or []):
        ...
        features = [
            str(item).strip()
            for item in raw_group.get("features", [])
            if str(item).strip()
        ]
        sequence_features = [
            str(item).strip()
            for item in raw_group.get("sequence_features", [])
            if str(item).strip()
        ]
        raw_pool_modes = raw_group.get("pool_modes", default_seq_pool_modes)
        pool_modes = [
            str(item).strip().lower() for item in raw_pool_modes if str(item).strip()
        ]
        if not features and not sequence_features:
            raise ValueError(
                f"semantic schema group '{name}' must define features or sequence_features"
            )
        if sequence_features and not pool_modes:
            raise ValueError(
                f"semantic schema group '{name}' has sequence_features but no pool_modes"
            )
        # 这里先保留“组名 + 普通特征名 + 序列特征名 + 需要展开的 pool modes”，
        # 还不直接生成真正传给 tokenizer 的 feature key。
        normalized.append(
            OrderedDict(
                [
                    ("name", name),
...
def build_rankmixer_semantic_groups(group_schema, default_seq_pool_modes=("mean",)):
    ...
    normalized_schema = normalize_rankmixer_group_schema(
        group_schema, default_seq_pool_modes=default_seq_pool_modes
    )
    for raw_group in normalized_schema:
        ...
        for seq_name in raw_group["sequence_features"]:
            for pool_mode in raw_group["pool_modes"]:
                # 历史序列在模型里会先被摘要成 seq::<name>::<mode> 这种 feature key；
                # semantic schema 需要把这种展开后的名字显式放进语义组里。
                expanded.append(f"seq::{seq_name}::{str(pool_mode).lower()}")
        semantic_groups.append(
            OrderedDict([("name", raw_group["name"]), ("features", expanded)])
        )
