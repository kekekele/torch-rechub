...
References:
    paper: RankMixer
    code: d:/code/keke/RankMixer/models/rankmixer_shen0118.py
"""
...
from .rankmixer_layers import LayerNorm, ParameterFreeTokenMixer, PerTokenFFN, PerTokenSparseMoE, SemanticTokenizer, gelu
...
class RankMixer(nn.Module):
    ...
    def _infer_tokenizer_input_dim(self):
        ...
        for fea in self.features:
            dims.append(fea.embed_dim)
        for fea in self.sequence_features or []:
            dims.append(fea.embed_dim)
    ...
    def _register_feature_projector(self, feature_name, input_dim):
        ...
        if input_dim == self.tokenizer_input_dim:
            self.feature_projectors[feature_name] = nn.Identity()
        else:
            self.feature_projectors[feature_name] = nn.Linear(input_dim, self.tokenizer_input_dim)
    ...
    def _build_feature_map(self, x):
        ...
        for fea in self.dense_features:
            ...
            feature_map[fea.name] = self._project_feature(fea.name, dense)
