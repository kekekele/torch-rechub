...
下面从多源异构特征编码、大规模样本表示、FiLM、双时间建模、`logQ`、课程式负样本难度调度和样本加权七个角度，说明当前实现为什么这样设计，以及它相对基础 HSTU 序列建模骨架额外增强了什么。

### 1. 多源异构特征编码

相对更传统、以 item/token 序列为主的 HSTU 输入范式，当前实现显式引入了 `user / item / context / time` 四类异构输入，并分别编码到统一表示空间。这可以看作本实现相对基础 HSTU 的一个关键增强点：模型不再只依赖“用户看过哪些 item”，而是同时利用“谁在看、看什么、在什么上下文下看、在什么时间看”这些条件信息。

其设计目的主要有四点：

1. `引入更丰富的监督信号`
  除 item 序列外，同时利用 user、context 和 time，使模型不仅建模序列共现关系，也建模用户偏好、场景条件和时间规律。
2. `按语义来源分治建模`
  user 特征更偏长期偏好，item 特征更偏内容语义，context 特征更偏当前请求条件，time 特征更偏周期规律；分路编码比直接将所有特征粗暴拼接更清晰。
3. `为条件序列建模提供结构化输入`
  后续 FiLM 用户条件调制、query-context 条件召回、RAB 时间偏置等机制，都依赖这些输入先被整理成结构化表示。
4. `提升真实推荐场景下的适用性`
  在本地优惠、吃喝团购等场景中，仅靠 item 序列通常不足以表达用户意图；多源异构特征编码更贴近真实业务决策过程。

从实现流程上看，这一模块可以概括为：

```text
[统一序列样本]
uid / iid / user feat / item feat / context / timestamp
              |
              v
      [按来源拆分四路输入]
    user       item       context       time
    |           |            |            |
    v           v            v            v
[用户特征编码] [物品特征编码] [上下文编码] [时间特征编码]
user id +      item id +     event/query   hour_sin/cos +
sparse/array   sparse/array  context feats wday_sin/cos
...
1. `信息表达更完整`
  模型不仅知道用户历史交互过哪些 item，还知道这些交互发生在什么场景、什么时间，以及用户自身具有什么属性或长期偏好。
2. `条件信息利用更自然`
  user、context、time 不再只是附属字段，而是分别承担用户条件建模、请求条件建模和时间规律建模的明确职责。
3. `降低特征混叠`
  分路编码减少了不同类型特征在输入早期相互干扰的风险，相比“所有特征直接拼接再过一个大 MLP”更稳定。
4. `更适合真实业务场景`
  在本地生活优惠场景下，商圈、时段、就餐场景、价格带等上下文往往和 item 本身同样关键，多源编码更符合业务实际。
5. `便于挂接后续增强机制`
  FiLM、双时间建模、query-context 条件召回等机制都可以自然建立在这套统一表示空间之上。

一个直观例子是：用户最近在工作日中午浏览过写字楼附近的快餐券、咖啡券，晚上又浏览过商场火锅双人餐。如果只有 item 序列，模型只知道“快餐、咖啡、火锅”这些 item；而有了多源异构编码后，模型还能同时理解这是哪个用户、发生在什么商圈和时段、对应的是午餐需求还是晚间聚餐需求，从而更准确地判断下一次最可能点击的本地优惠。

相对基础 HSTU 而言，这里的增强点在于：当前实现将原本偏 item-sequence-driven 的输入方式，扩展为**多源异构条件输入驱动的序列表征框架**。基础 HSTU 更强调时序编码主干，而本实现进一步增强了“输入如何被结构化表达”这一层。

### 2. 面向大规模样本的索引化表示

当前实现并不是直接把原始表数据一次性加载成内存中的稠密训练样本，而是先经过三步转换：

1. `统一接口与 schema 显式化`
  先把输入约束在 `seq / user / item / data_format` 的统一结构下，再通过 `feature_columns + data_format.csv` 生成显式的 `feature_schema`，明确本次训练真正使用的字段与类型。
2. `索引映射压缩`
  将 `uid`、`iid` 和离散特征值映射为从 1 开始的连续整数 id，其中 `0` 统一保留给 padding、缺失值和未知值。这样做把原始字符串或高基数离散值压缩到紧凑整数空间，便于 embedding 查表和序列存储。
3. `JSONL + offsets` 组织
  序列样本按用户写入 `train_seq.jsonl / valid_seq.jsonl / predict_seq.jsonl`，再额外生成 `*_seq_offsets.pkl` 记录每一行在文件中的字节偏移。Dataset 训练时只需根据 offset 直接 seek 到对应样本，而不必把整份序列文件预读进内存。

这一设计的核心不是“简单做了 ID 化”，而是构造了一种更适合大规模召回训练的数据表示层：

1. `显著降低内存压力`
  原始字符串 id、原始稀疏取值和完整 DataFrame 在大规模场景下内存开销很高；映射成连续整数并写成 JSONL 后，可以把样本以更轻量的方式落盘，只在取 batch 时读取需要的行。
2. `支持流式写出与随机读取`
  数据构建阶段可以顺序扫描、顺序写出；训练阶段则依赖 offsets 做随机访问。这样兼顾了离线预处理的吞吐效率和训练时按样本索引读取的灵活性。
3. `便于跨 split 与多阶段复用`
  `train/valid/predict` 三套样本、`predict_set` 候选集、`user_query_context` 和 `meta.json` 都共享同一套 indexer 与 schema，避免训练和推理阶段出现字段语义不一致的问题。
4. `有利于长尾与高基数字段建模`
  无论是用户 id、物品 id，还是高基数离散 context 特征，连续 id 映射都能直接转成 embedding 索引，不需要保留原始值的稀疏表示结构。
5. `更贴合召回场景的数据生命周期`
  召回训练通常需要反复扫描大量序列样本，而推理又需要复用同一套 item 索引与候选表示。当前这套组织方式天然适合“离线构建一次，多次训练/评估/推理复用”的流程。

一个直观例子是：如果原始数据里 `uid`、`iid` 和若干 context 字段都是字符串，且总样本量达到千万级，那么直接把所有样本常驻内存会非常昂贵。当前实现会先把这些字段转成紧凑整数 id，再把每条序列样本顺序写入 JSONL，并通过 offsets 记录位置。训练时即使只需要第 135792 条样本，也可以直接定位到那一行读取，而不必把前面的所有样本全部加载出来。

相对更基础的序列建模实现而言，这里的增强点在于：当前实现显式引入了**面向大规模样本的索引化表示层**，把数据组织方式本身做成训练系统的一部分，而不是默认依赖一次性载入内存的数据管道。
...
一个直观例子是：同一条“本地火锅优惠套餐”对两个用户的语义重点可能不同。对偏夜宵和聚餐的用户，FiLM 可能放大与多人套餐、晚间时段、商圈距离相关的表示维度；对偏工作日午餐的用户，FiLM 可能放大与单人套餐、出餐速度、价格带相关的表示维度。优惠主体仍然是同一个团购商品，但序列编码会在用户条件下发生有控制的偏置。
...
1. `绝对时间例子`
  用户经常在工作日上午 11:30 左右打开本地优惠 App 浏览午餐团购、咖啡券和快餐套餐。周一 11:28 和周二 11:35 的行为虽然日期不同，但在周期时间特征空间里很接近，模型能学到“这是同一种午餐时段的本地消费模式”。
2. `相对时间例子`
  用户 30 天前看过一张奶茶优惠券，10 分钟前连续浏览了火锅团购、烤肉双人餐和商场美食代金券，当前要预测下一次点击。对当前预测位置而言，10 分钟前这组“即时觅食”行为与当前时间差更小，RAB 会更倾向于提高它们的 attention 影响力；30 天前的奶茶券记录仍可作为长期偏好线索，但通常影响更弱。
...
当前实现没有在整个训练过程中固定使用同一难度的负样本，而是通过 `curriculum_schedule` 按训练进度动态调整 `hardtopk` 和 `glb_neg_ratio`。在 `train.py` 中，`set_curriculum` 会根据当前 `global_step / total_steps` 选择当前阶段的配置；在 `model.py` 中，`hard_topk` 又会进一步控制从全局负样本和批内负样本中保留多少个“最难”的负样本参与归一化。

其核心思想是：训练早期先让模型区分较容易的负样本，等模型具备基础判别能力后，再逐步引入更难、更接近正样本的负样本，从而实现由易到难的学习过程。

这种做法的优势主要有三点：

1. `降低训练早期不稳定性`
  如果模型一开始就只看到大量困难负样本，loss 往往震荡更大，学习边界也更不稳定。课程式调度先从更温和的对比环境开始，有利于模型先学到基础排序能力。
2. `逐步提升精细区分能力`
  随着训练推进，`hardtopk` 会逐渐启用并缩小到更难的一批负样本，模型被迫在更相似的候选之间学习更细的判别边界。
3. `兼顾全局负样本与批内负样本`
  通过 `glb_neg_ratio` 控制来自全局候选和批内候选的比例，模型可以同时利用“分布更广的全局干扰项”和“语义更接近的批内干扰项”。

一个直观例子是：

- 训练初期，模型可能主要接触与正样本差异较大的负样本，先学会粗粒度地区分“相关”和“不相关”。
- 训练后期，模型开始更多关注与正样本语义接近、分数也更高的负样本，例如相似品类、相似主题或 embedding 空间中距离更近的 item，从而提升精细召回能力。

相对基础 HSTU 而言，这里的增强点不在主干结构，而在**训练过程控制**：当前实现不把负样本机制视为静态配置，而是把负样本难度设计成随训练阶段演化的课程式策略。

### 7. `action_nonzero_weight` 样本加权

当前实现允许对 `action != 0` 的 query context 样本施加额外权重。具体做法是在 `model.py` 中读取 query context 对应的 action id，对非零 action 样本赋予更大的 loss 权重，然后对 loss 做加权平均。

它的核心目的，是让模型在统一训练目标下进一步显式强调某类业务更重要的样本，而不是默认所有训练位置同权。

这样做的优势主要有三点：

1. `突出高价值行为`
  如果某些 `action` 表示更强的用户意图，例如点击、收藏、加购、转化等，那么提高这些样本权重可以让模型更关注高价值目标，而不是被大量弱行为样本主导。
2. `缓解样本分布不均`
  在真实数据中，强行为通常占比较低。如果完全按样本数平均，模型更容易主要拟合高频弱行为；加权后可提高强行为样本在梯度中的占比。
3. `更贴合业务目标`
  有些召回任务并不只是追求整体平均指标，而是更关心特定行为子集的召回质量。样本加权把这种偏好显式写进训练目标。

一个直观例子是：

- 若 `action=0` 表示普通上下文或弱意图行为，`action=1/2` 表示更强的交互意图，那么将 `action_nonzero_weight` 设为 `2.0` 或 `3.0` 后，这些强意图样本对应的误差会在训练中产生更大影响。
- 结果上，模型可能牺牲少量整体平均最优性，但换来在高价值行为子集上的更好召回表现。

相对基础 HSTU 而言，这里的增强点在于：当前实现把**任务感知的样本重要性**显式注入损失函数，而不是把所有监督样本视为完全同质的训练信号。

### 8. 相对基础 HSTU 的整体增强总结

如果将基础 HSTU 看作“负责时序建模的主干骨架”，那么当前实现主要做了八类增强：

1. `输入建模增强`
  通过 user/item/context/time 四路异构特征编码，将输入从以 item token 为主扩展为多源条件输入驱动的统一表示框架。
2. `数据系统增强`
  通过统一 schema、连续 id 映射、`jsonl + offsets` 和共享 indexer，把数据侧扩展为适合大规模序列召回训练的索引化表示层。
3. `条件化增强`
...
7. `训练过程增强`
  通过课程式负样本难度调度，使模型能够从粗粒度区分逐步过渡到细粒度区分，提升训练稳定性与后期精细判别能力。
8. `监督信号增强`
  通过 `action_nonzero_weight` 对高价值行为样本施加更大权重，使优化目标更贴近业务关注的关键行为子集。
